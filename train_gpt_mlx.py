...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        self.attn_window = int(os.environ.get("ATTN_WINDOW", "0"))  # 0 = full causal (unchanged)
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        if self.attn_window and self.attn_window > 0:
            # Restrict each query to the most recent `attn_window` keys (still causal).
            # Turns attention cost from O(seqlen^2) into O(seqlen * window).
            rows = mx.arange(seqlen)[:, None]
            cols = mx.arange(seqlen)[None, :]
            keep = (cols <= rows) & (rows - cols < self.attn_window)   # causal AND within window
            mask = mx.where(keep, mx.array(0.0, dtype=q.dtype), mx.array(-1e9, dtype=q.dtype))
...
class GPT(nn.Module):
    # - factorized token embedding (small V x rank table + shared rank x dim projection) + RMSNorm
    # - encoder half accumulates skip tensors
    # - decoder half consumes reversed skips with learned skip_weights
    # - tied embeddings for the LM head (the LM head reuses the same factorized table)
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        # Factorized embedding: one V x dim table is replaced by a small V x rank table
        # plus a shared rank x dim up-projection. Storage drops from V*dim to rank*(V + dim),
        # which only helps when rank < V*dim/(V+dim) (e.g. at vocab 1024, dim 512: rank < ~341).
        self.embed_rank = int(os.environ.get("EMBED_RANK", "128"))
        self.tok_emb_small = nn.Embedding(vocab_size, self.embed_rank)   # V x rank
        self.tok_emb_proj = CastedLinear(self.embed_rank, dim)           # rank -> dim (weight shape: dim x rank)
        ...
        self.recurrence = int(os.environ.get("RECURRENCE", "1"))

        for b in self.blocks:
            b.attn.proj.weight = mx.zeros_like(b.attn.proj.weight)
            b.mlp.proj.weight = mx.zeros_like(b.mlp.proj.weight)
        self.tok_emb_small.weight = (
            mx.random.normal(self.tok_emb_small.weight.shape, dtype=mx.float32) * tied_embed_init_std
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        # Look up the small rank-dim vector, then project it up to the full model dim.
        x = rms_norm(self.tok_emb_proj(self.tok_emb_small(input_ids)).astype(COMPUTE_DTYPE))
        x0 = x
        skips: list[mx.array] = []
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        # Cross-entropy over flattened tokens. The tied LM head reuses the factorized embedding:
        # logits = x @ proj_weight @ small_weight.T, which equals x @ E.T for the effective
        # table E = small_weight @ proj_weight.T. tok_emb_proj.weight has shape (dim, rank),
        # so shape[0] = dim is the residual width we reshape to.
        x = self(input_ids).reshape(-1, self.tok_emb_proj.weight.shape[0])
        y = target_ids.reshape(-1)
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            logits_proj = x @ self.tok_emb_proj.weight.astype(x.dtype) @ self.tok_emb_small.weight.astype(x.dtype).T
        ...
        for s in range(0, n, self.logit_chunk_tokens):
            ...
            logits_proj = x[s:e] @ self.tok_emb_proj.weight.astype(x.dtype) @ self.tok_emb_small.weight.astype(x.dtype).T
            logits = self.softcap(logits_proj)
...
class SplitOptimizers:
    ...
    def __init__(self, model: GPT, args: Hyperparameters):
        ...
        # Factorized embedding: the small V x rank table is the "embedding" (Adam, embed LR);
        # the rank x dim up-projection is a plain 2D matrix, so it joins the Muon matrix group.
        self.embed_key = "tok_emb_small.weight"
        self.matrix_keys = [
            k
            for k, p in params.items()
            if k == "tok_emb_proj.weight"
            or (k.startswith("blocks.") and p.ndim == 2 and not any(pattern in k for pattern in CONTROL_TENSOR_NAME_PATTERNS))
        ]
        self.scalar_keys = [
            k
            for k, p in params.items()
...
def quantize_float_array(arr: mx.array) -> tuple[np.ndarray, np.ndarray]:
    qmax = (2 ** (int(os.environ.get("QUANT_BITS", "8")) - 1)) - 1
    f32 = _np_float32(arr)
    if f32.ndim == 2:
...
def eval_val(
    ...
    _val_max_seqs = int(os.environ.get("VAL_MAX_SEQS", "0"))
    if _val_max_seqs > 0:
        total_seqs = min(total_seqs, _val_max_seqs)
...
def main() -> None:
    ...
    log(
        f"dtypes tok_emb:{model.tok_emb_small.weight.dtype} "
